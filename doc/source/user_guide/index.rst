...
* Log messages can be sent directly to the MI Data Flow instance via :meth:`~.MIDataflowIntegration.log_msg_to_instance`::

    dataflow_integration = MIDataflowIntegration()
    dataflow_integration.log_msg_to_instance("Script started", level="Info")

  Log messages generated by this method are added to the Data Flow log via the Data Flow API. Contrary to the previous
  approach, these messages are available immediately, rather than when the script completes. This can be useful to
  report progress during long-running scripts.
